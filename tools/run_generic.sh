...
for t in "$@"; do
  ...
  python3 tools/render_theme.py /home/claude/private/dense-real.json $t out/gen/$t/base.pdf --density $d >/dev/null 2>&1
  python3 tools/ats_generic.py out/gen/$t/base.pdf >> out/gen/$t.txt 2>&1
  for m in short-name no-research-honors one-job-no-summary flat-skills; do
    ...
    python3 tools/ats_generic.py out/gen/$t/$m.pdf --expect "$(cat out/mut/$m.expect.json)" >> out/gen/$t.txt 2>&1
  done
